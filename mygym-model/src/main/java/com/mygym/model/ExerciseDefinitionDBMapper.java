...
@DMLQueries(
	update="update exercise_definition set name=?,video_url=?,description=? where exercise_definition_id=?",
	updateRowid="",
	delete="DELETE FROM exercise_definition WHERE exercise_definition_id=?",
	deleteRowid="",
	insert="insert into exercise_definition (name,video_url,description) values (?,?,?)",
	selectone="SELECT t.exercise_definition_id, t.name, t.video_url, t.description FROM  exercise_definition t  WHERE exercise_definition_id=?",
	selectall="SELECT t.exercise_definition_id, t.name, t.video_url, t.description FROM  exercise_definition t ",
	pagingSelect="SELECT t.exercise_definition_id, t.name, t.video_url, t.description FROM  exercise_definition t  {criteria} LIMIT {offset}, {pageSize}")
public class ExerciseDefinitionDBMapper extends MySQLDBMapper  {
	...
	*/
	...
	public void saveChildren(ModelObject mo) {
		...
		if (ret.equipmentNeededLoaded()){
			EquipmentExerciseDBMapper equipmentneededMapper= new EquipmentExerciseDBMapper(this.getConnection());
			equipmentneededMapper.saveModelObjectList(ret.getEquipmentNeeded());
		}
	...
	public void fillStatement(ModelObject obj, CallableStatement stmt, boolean bForInsert) throws Exception {
		...
		if (bForInsert){
		 ...
		 /* For Update statement*/
			stmt.setInt(4,o.getExerciseDefinitionId().intValue());
		}
