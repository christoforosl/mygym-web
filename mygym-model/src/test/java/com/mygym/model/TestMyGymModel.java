...
import com.mygym.model.EquipmentExercise;
import com.mygym.model.EquipmentExerciseDBMapper;
	...
	public void testExerciseDefinition() throws Exception  {
		...
		try {
			...
			if (newvals !=null ) {
				...
				// ExerciseDefinitionNew={"Name":"","VideoUrl":"","Description":"", }
...
//*** below we are testing loading of associated objects:
    ...
    @Test
	public void testEquipmentExercise() throws Exception  {
        
		ApplicationService.beginTrans();
		System.err.println("testing EquipmentExercise"  );
		try {
			
			final EquipmentExerciseDBMapper pdb = new EquipmentExerciseDBMapper();
			final String newvals = (String) testProps.get("EquipmentExerciseNew");
			
			if (newvals !=null ) {
				// to test put this in nuafTests.properties
				// EquipmentExerciseNew={"EquipmentId":"","ExerciseId":"", }
				JSONObject jsn = new JSONObject(newvals);
				EquipmentExercise newmo = new EquipmentExercise();
			...
			newmo.setExerciseId(jsn.optString("ExerciseId",null));
				...
				final Object pid = DBUtils.getValue("select equipment_exercise_id from equipment_exercise  limit 1,1");
				if(pid!=null) {	

					EquipmentExercise p = (EquipmentExercise)pdb.findOne("equipment_exercise_id=?",pid);
					final EquipmentExercise p2 =  (EquipmentExercise)p.clone();			
...
//*** below we are testing loading of associated objects:
			p.getEquipment();
					...
					//now reload object from database
					p = (EquipmentExercise)pdb.findOne("equipment_exercise_id=?",pid);
					Assert.assertNotNull( p );
				} else { 
					System.err.println("  --Warning: No records found of type EquipmentExercise"  );
				}
